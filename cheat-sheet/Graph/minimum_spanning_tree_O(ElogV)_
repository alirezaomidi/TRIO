...
//Variants:
// 1. for finding maximum spanning tree just multiply weights of edges b -1 and run the alg.
...
// 4. in order to find second best spanning tree, first find MST, after that for each edge in MST temporary set it off so the kruskal alg wond select it, do this for every edge in MST and the best answer is second best spanning tree.
...
// in main:
// call buildUfds(n) where n is number of verteces before calling kruskal alg.
